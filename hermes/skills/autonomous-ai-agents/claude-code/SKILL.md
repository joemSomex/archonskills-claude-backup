...
Skills use `.claude/skills/<skill-name>/SKILL.md` for project scope or `~/.claude/skills/<skill-name>/SKILL.md` for personal scope. Claude can load a matching skill automatically, and users can invoke it explicitly as `/skill-name`. Current documentation merges custom commands into skills; do not use a flat `.claude/skills/name.md` file for new skills.

```markdown
---
name: database-migration
description: Plan and verify database schema migrations.
---
When asked to create or modify database migrations:
1. Discover and use the project's existing migration tooling.
2. Plan compatibility and recovery; use rollback or roll-forward as appropriate.
3. Test against a disposable database with realistic constraints.
```

Keep large source corpora in supporting files and load them selectively. When adapting a Hermes skill for Claude Code, replace Hermes-only tool names and metadata; copying the directory alone does not adapt its workflow. Verify format against https://code.claude.com/docs/en/skills and distinguish package integrity checks from actual Claude runtime testing.
