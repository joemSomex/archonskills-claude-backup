...
1. **Always pass `--detach` when the request came from chat.** These runs take
   ...
   to watch it. On this root-owned Hermes host, prefix launches with
   `IS_SANDBOX=1`; otherwise Claude Code SDK rejects `bypassPermissions` before
   the first workflow node executes. This environment flag is the supported
   root/sandbox path and does not modify the pinned engine.
